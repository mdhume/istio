...
func (a v1alpha1PolicyApplier) InboundFilterChain(sdsUdsPath string, meta map[string]string) []plugin.FilterChain {
	...
	if mtls.GetMode() == authn_v1alpha1.MutualTls_PERMISSIVE {
		log.Info("Allow both, ALPN istio and legacy traffic")
		...
		if certsPath, ok := meta[model.NodeMetadataDNSCert]; ok {
			log.Info("Metadata contains DNS_CERT, adding additional filter chain with transport_protocols filterchainmatch")
